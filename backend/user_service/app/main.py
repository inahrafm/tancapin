...
function App() {
  ...
  const handleLogout = () => {
    ...
    setCurrentPage('login'); // Redirect ke halaman login
  };
  ...
  const renderContent = () => {
    ...
    switch (currentPage) {
      ...
      case 'login': // Jika sudah login tapi mencoba ke /login lagi, tampilkan halaman Login/Register (bisa untuk ganti user)
        return <LoginRegisterPage onLoginSuccess={handleLoginSuccess} />;
      default:
